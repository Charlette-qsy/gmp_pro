...
ctrl_gt pwm_out_pu;
extern uint32_t adc_results[8];
// Boost Controller Suite
boost_ctrl_t boost_ctrl;
uint32_t clock_start=0;
pid_regular_t voltage_loop;
pid_regular_t current_discharge_reference;
pid_regular_t current_charge_reference;
//
...
void ctl_init()
{
    ctl_init_pid(&voltage_loop, 0.4, 0.01, 0, CONTROLLER_FREQUENCY);
    ctl_init_pid(&current_discharge_reference, 0.1, 0.05, 0, CONTROLLER_FREQUENCY);
    ctl_init_pid(&current_charge_reference, 0.5, 0.01, 0, CONTROLLER_FREQUENCY);
    ctl_init_boost_ctrl(
        // Boost controller
        &boost_ctrl,
...
uint16_t sgen_out = 0;
//uint16_t AD_voltage[101];
uint32_t AD2_voltage_sum=0;
uint16_t  time_sample=0;
uint16_t AD2_voltage_dispaly=0;
void ctl_mainloop(void)
{
	uint8_t i;
	//AD_voltage[time_sample]=adc_results[0];
	
	if(time_sample%10==0){
		AD2_voltage_sum=AD2_voltage_sum+adc_results[1];
	}
	time_sample++;
	if(time_sample==1001){
	 time_sample=0;
	 AD2_voltage_dispaly=(uint16_t)(AD2_voltage_sum/100);
	 MzShowShort(0,31,AD2_voltage_dispaly,1);
   AD2_voltage_sum=0;		
	}
	
    //MzShowShort(0,31,adc_results[0],1);
